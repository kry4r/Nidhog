...
namespace nidhog::graphics::d3d12::upload 
{
    namespace 
    {
        //与Command Frame的区别是我们每个Frame都有一个cmd_list
        //多线程每个线程都需要自己的cmd_list
        struct upload_frame
        {
        ...
        //定义多少个线程可以同时记录复制命令
        constexpr u32       upload_frame_count{ 4 };                                 //定义多少个线程可以同时记录复制命令
        upload_frame        upload_frames[upload_frame_count]{};                    
        ID3D12CommandQueue* upload_cmd_queue{ nullptr };                             //利用CommandQueue来记录，自然也要定义它的fence相关的东西
        ...
        u32 get_available_upload_frame()
        {
            ...
            // 没有Frame上传完成，我们在这只有一个线程
            // so we can iterate through frames until we find one that is ready.
