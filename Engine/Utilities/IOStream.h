...
namespace nidhog::utl {
    //确保每次buffer从正确位置读取
    // NOTE: (Important) 该Utilitie Program仅供本地使用 (i.e. within one function).
    //       不要将instances保留为member variables。
    ...
    public:
        ...
        // This template function is intended to read primitive types (int, float, bool)
        template<typename T>
        [[nodiscard]] T read()
        {
            T value{ *((T*)_position) };
        ...
        // reads 'length' bytes into 'buffer'. The caller 负责在buffer中分配足够内存
    ...
    public:
        ...
        // This template function is intended to write primitive types (int, float, bool)
        template<typename T>
        void write(T value)
        {
