...
namespace nidhog::id {
    ...
    using generation_type = std::conditional_t<internal::generation_bits <= 16, std::conditional_t<internal::generation_bits <= 8, u8, u16>, u32>;//储存的generation的类型·
    //加上两个断言判断，防止数据溢出
        ...
        new_generation(id_type id)//函数以得到id
    {
        ...
        assert(generation < (((u64)1 << internal::generation_bits) - 1));//计算最大值并进行判断
